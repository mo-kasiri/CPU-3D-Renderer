...
void process_input(void)
{
	...
	case SDL_KEYDOWN:
		...
		if (event.key.keysym.sym == SDLK_9)
			render_method = RENDER_TEXTURE_WIRE_VERTEX;
		...
		if (event.key.keysym.sym == SDLK_d)
			mesh.translation.x += 10.0 * delta_time;
...
void render(void)
{
	...
	for (int i = 0; i < array_length(triangles_to_render); i++)
	{
		...
		// Drawing Wires
		if (render_method == RENDER_WIRE || render_method == RENDER_WIRE_VERTEX || render_method == RENDER_FILL_TRIANGLE_WIRE || render_method == RENDER_FILL_TRIANGLE_WIRE_VERTEX || render_method == RENDER_TEXTURE_WIRE || render_method == RENDER_TEXTURE_WIRE_VERTEX)
		{
